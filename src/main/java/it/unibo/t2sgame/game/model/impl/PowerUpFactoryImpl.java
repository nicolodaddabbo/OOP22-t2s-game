...
public class PowerUpFactoryImpl implements PowerUpFactory{
    ...
    public PowerUp generateDamageBoostPowerUp() {
        return fromFunction(entity -> entity.getComponent(ShootComponent.class).ifPresent(c -> c.setProjectileDamage(c.getProjectileDamage() + 1)));
    }

    @Override
    public PowerUp generateProjectileSpeedUpPowerUp() {
        return fromFunction(entity -> entity.getComponent(ShootComponent.class).ifPresent(c -> c.setProjectileSpeed(c.getProjectileSpeed() + 1)));
    }

    @Override
    public PowerUp generateProjectileSizeUpPowerUp() {
        return fromFunction(entity -> entity.getComponent(ShootComponent.class).ifPresent(c -> c.setProjectileSize(c.getProjectileSize() + 5)));
    }

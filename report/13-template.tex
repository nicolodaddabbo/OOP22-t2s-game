...
Quando il giocatore inizia una partita di Time2Survive si ritroverà catapultato nel mondo di gioco.
\\
Una partita (\textbf{Game}) gestisce il proprio stato (\textbf{State}) e il mondo di gioco di T2S.
%
Il mondo (\textbf{World}) di T2S è l'arena dove i diversi personaggi (\textbf{Character}) si affrontano.
\\
Al giocatore sarà data la possibilità di controllare (\textbf{CharacterController}) un personaggio (\textbf{Player}).
\\
Durante la partita verranno generate ondate (\textbf{Wave}), ovvero un gruppo di nemici che il giocatore dovrà affrontare.
%
Un nemico (\textbf{Enemy}) è un personaggio comandato da un'intelligenza artificiale con il compito di attaccare e uccidere il player.
\\
Per combattere i nemici, il player avrà la possibilità di sparare dei proiettili (\textbf{Projectile}) che possono essere migliorati tramite potenziamenti (\textbf{Power up}).
\\
...
\\
All’inizio della partita al player verranno assegnati dei proiettili di default e un determinato numero di punti vita.
%
